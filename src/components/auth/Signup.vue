  ...
  <div class="signup">
    <div class="min-h-screen flex items-center justify-center bg-gray-100 px-4">
      ...
      >
        ...
        <!--/end-->
        <Confirmaccount v-if="confirm" :item="item" :loading="loading" />
        <Registeraccount v-else :item="item" :loading="loading" />
      </div>
    </div>
  </div>
</template>
...
import Registeraccount from "./@utils/Registeraccount.vue";
export default {
  name: "Signup",
  components: {
    Confirmaccount,
  ...
  methods: {
    ...
    registeraccount() {
      this.loading = true;
      // 1. dispatch to vuex
      this.$store.dispatch("auth/register", this.item, { root: true });
      // 2.  clear password
      this.item.password = null;
      this.text = "Confirm your account";
      this.confirm = true;
